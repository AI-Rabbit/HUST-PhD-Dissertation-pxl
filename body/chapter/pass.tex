\chapter{捏合天线波束赋形性能分析与几何辅助定位算法设计}\label{ch:pass}

\section{引言}\label{sec:pass_intro}

在未来的 6G 通信网络中，物理层的设计目标不仅包括高速数据传输，还涵盖高精度感知与定位。前两章探讨的DMA主要通过调控单元的电磁响应来实现信道重构。近年来，学术界还提出了一类直接改变天线物理位置来重塑信道的新型PASS架构。它借助介质波导结构，使 PA 能够在米级范围内滑动并激活，从而主动规避障碍物，建立 LoS 链路。

现有 PASS 研究主要关注通信性能，如阵列增益表征\cite{ouyang2025array}与波束赋形优化\cite{wang2025modeling}。然而，在这些波束赋形设计中，一个隐含的前提是基站需要先获知用户的位置。因此，如何利用 PASS 的重构特性来实现高精度的定位，是一个亟待解决的问题。目前，少数关于 PASS 定位的研究采用固定天线阵列配合加权最小二乘算法，未能利用大尺度移动带来的几何构型重构能力\cite{zhang2025pinchingantenna}。同时有研究指出PASS通信最优与感知最优的天线布局存在矛盾\cite{jiang2025pinching}。在用户位置未知或动态变化时，若维持固定通信阵列构型进行定位，系统会面临严重的几何精度衰减，导致不同区域的定位误差分布不均匀。因此，研究定位导向的最优空间几何构型，并设计自适应天线布局策略，是发挥 PASS 通感一体化潜力的关键。

针对上述问题，本章从理论上刻画了 PASS 在最优通信对齐策略下的大尺度覆盖能力，探索了其在定位感知任务中的理论与算法。本章的主要研究内容归纳如下：

\begin{enumerate}
	\item 推导了 PASS 通信系统的理论性能。在最优天线对齐策略下，给出系统平均信噪比闭式解；结合大阵列渐近近似，推导空间中断概率的解析表达式。
	
	\item 建立基于 FIM 的 PASS 定位优化框架。针对通信与感知在天线构型上的矛盾，从最大化 FIM 迹的角度出发，证明了圆形几何构型可提供最优的定位精度，同时推导了该圆周半径的闭式表达式。
	
	\item 提出了基于圆形几何的交替天线配置与定位算法，将二维 MUSIC 定位与最优圆形阵列重构相结合，通过交替迭代修正估计的用户位置并驱动 PA 向最优排布滑动。通过数值仿真验证了 CG-APPL 算法在不同实验条件下均能实现优于固定几何基准方案的定位精度。
\end{enumerate}

本章剩余部分组织如下：第~\ref{sec:pass_system_model}节介绍 PASS 上行系统模型；第~\ref{sec:pass_performance}节推导最优对齐策略下的平均信噪比与空间中断概率；第~\ref{sec:pass_CG-APPL}节基于 FIM 理论推导定位导向的最优圆形天线分布规律，并阐述 CG-APPL 算法流程；第~\ref{sec:pass_sim}节通过数值仿真验证系统通信性能，并评估所提定位算法的精度与鲁棒性；第~\ref{sec:pass_conclusion}节对本章研究内容进行总结。


\section{系统模型}\label{sec:pass_system_model}
...
\section{最优天线位置与理论性能分析}\label{sec:pass_performance}
...
\section{基于圆形几何的交替天线配置与定位算法}\label{sec:pass_CG-APPL}
...
\section{数值仿真分析}\label{sec:pass_sim}
...
\section{本章小结}\label{sec:pass_conclusion}

本章围绕PASS的大尺度空间重构特性，分析了其在通信波束赋形与辅助定位中的性能边界，并讨论了通信最优与感知最优几何构型之间的折中关系。通信方面，在所有天线对齐至用户横坐标的最优配置策略下，推导了系统平均信噪比与空间中断概率。渐近分析表明，平均信噪比随波导数量 $M$ 线性增长；通过动态规避远端用户的大尺度路径损耗，PASS 的覆盖性能优于传统固定阵列。
感知定位方面，指出了通信最优的共线构型会导致空间分集缺失。基于克拉美-罗下界分析，推导了含位置梯度的费雪信息矩阵。在最大化 FIM 迹的准则下，证明了实现高精度定位的最优几何构型是以目标正上方为圆心、半径近似等于波导部署高度 $L_z$ 的圆形分布。
据此提出了 CG-APPL 算法，通过二维 MUSIC 空间谱估计与天线圆形重构的交替迭代来逼近最优观测几何。数值仿真表明，在通信可靠性方面，为达到 $10^{-3}$ 的空间中断概率，PASS 较同等规模的固定线阵可节省约 7.5 dB 的发射功率；在定位精度方面，CG-APPL 算法克服了固定阵列径向分辨率差的问题，在服务区域深度扩展至 20 m 的条件下，均方根误差仍维持在 $10^{-3}$ m 量级，消除了边缘与角落的定位盲区，有效提升了感知精度。