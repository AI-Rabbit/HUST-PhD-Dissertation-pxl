\chapter{连续相移DMA波束赋形性能分析与相位对齐算法设计}\label{ch:cdma}

\section{引言}\label{sec:cdma_intro}

根据前文的介绍可知，DMA技术通过将包含可调谐超材料单元的平面阵列与传统的波导馈电技术相结合，能够以较少的射频链路驱动大规模天线阵列。其在硬件成本和功耗方面的优势使其成为 6G 物理层架构演进的关键技术之一。近年来，学术界针对DMA辅助的无线通信系统开展了广泛的研究，涵盖了上下行传输\cite{jiang2022hybrid}、全息波束赋形\cite{altinoklu2025investigation}、近场波束聚焦\cite{gavriilidis2025near}等场景。

然而，现有关于DMA波束赋形的研究在理论模型和算法设计上仍存在一些局限。
首先，DMA 辐射单元的幅相响应由洛伦兹谐振决定，幅度和相位非线性耦合，不能独立调节。现有文献往往将约束理想化为恒定幅度，求得松弛解后再投影回实际可行域。这种近似处理破坏了相位对齐条件，会带来波束增益损失。第二，信号在波导中传播伴随着介质衰减，系统性能受波导衰减和阵列规模双重影响。现有研究大多停留在数值仿真，缺乏考虑波导物理特性的闭式性能分析，难以从理论上指导实际DMA系统的天线规模配置。最后，传统波束赋形高度依赖精确的CSI，对于数千个单元的DMA来说，估计CSI的庞大导频开销在实际工程中是难以接受的。

本章针对上述问题，在考虑DMA洛伦兹约束的前提下，深入开展连续相移条件下的波束赋形性能分析与低开销算法设计。本章的主要研究内容归纳如下：

\begin{enumerate}
	\item 提出了DMA最优相位配置方法。通过对洛伦兹幅相耦合约束进行数学解耦，推导出连续相移条件下实现最大有效信道增益的最优相位闭式解，避免了松弛投影方法带来的相位失配问题。
	
	\item 建立了涵盖波导衰减与洛伦兹约束的DMA系统理论分析框架。推导了系统平均信噪比、空间中断概率和遍历容量的解析表达式。从理论上揭示了系统容量随单条波导辐射单元数增加呈现出先升后降的非单调缩放规律，还证明了所提最优配置方法在相干增益上严格优于现有的直接映射与欧氏投影松弛算法，并量化了非完美CSI对波束赋形增益的影响。
	
	\item 针对大规模 DMA 阵列 CSI 获取困难的工程问题，提出了基于 RSSI 的波束赋形方案。通过建立单个单元相位优化的回归模型，设计了低开销的RECD算法；然后利用全局参考信号做自干扰剔除，进一步提出NI-REPA算法，实现了无显式CSI条件下的快速寻优。
\end{enumerate}

本章剩余部分组织如下：第~\ref{sec:cdma_system_model}节介绍本章考虑的DMA辅助下行通信系统模型；第~\ref{sec:cdma_opt}节推导洛伦兹约束下的最优相位闭式解，并对系统的平均信噪比、中断概率与遍历容量进行闭式推导与渐近分析；第~\ref{sec:cdma_rssi}节在没有显式 CSI 的假设下，设计并提出基于RSSI反馈的RECD与NI-REPA波束赋形算法；第~\ref{sec:cdma_sim}节通过数值仿真验证理论推导的准确性以及所提算法的有效性；第~\ref{sec:cdma_conclusion}节对本章研究内容进行总结。
...
\section{基于RSSI的相位估计与相位对齐算法设计}\label{sec:cdma_rssi}
...
\section{数值仿真分析}\label{sec:cdma_sim}
...
系统工作于典型的微波频段，载波频率设定为 $f_c = 2.6 \text{ GHz}$。基站的发射功率设定为 $P_t = 30 \text{ dBm}$，用户端的接收噪声功率为 $\sigma^2 = -100 \text{ dBm}$，基站与用户之间的距离为 $d = 100 \text{ m}$。大尺度衰落采用3GPP城市微小区路径损耗模型，其路损值（单位：dB）的计算公式为：
\begin{equation}
...
\section{本章小结}\label{sec:cdma_conclusion}

本章针对DMA波束赋形中的洛伦兹幅相耦合约束、波导衰减效应以及大规模阵列下 CSI 获取开销大的难题，开展了连续相移条件下的性能分析与低开销算法设计。理论层面，通过解耦洛伦兹约束推导了最优连续相位的闭式解，并构建了系统平均信噪比、中断概率与遍历容量的解析框架。分析得出，系统波束增益随单波导单元数 $N_e$ 增加呈现先升后降的规律，在弱衰减近似下，最佳单元数为 $N_e^\star \approx 1.256/(\alpha D)$。同时，渐近分析证明了所提最优配置在相干增益上优于松弛方法。相较于直接映射松弛方法，相对增益可达$20\%$至$70\%$。算法层面，提出了无需显式 CSI 的盲波束对齐方案。通过建立相位估计的线性回归模型，从理论上证明了均匀采样能消除参数估计耦合。仿真结果验证了理论分析的准确性，表明在采样点数 $M \ge 4$ 时即可获得各向同性且最小的均方误差。
相较于随机搜索，所提的RECD与NI-REPA算法收敛更快，特别是利用全局参考剔除自干扰的NI-REPA算法，在 40 dBm 发射功率下仅需一轮测量即可达到  $95\%$ 以上的归一化接收功率。
