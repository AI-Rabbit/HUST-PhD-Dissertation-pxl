...
\begin{table}[!htbp]
	...
	\begin{tabular}{cccc}
		\toprule
		\textbf{方法} & \textbf{映射策略} & \textbf{相干增益系数} & \textbf{归一化性能} \\
		\midrule
		最优相位配置 & 式~\eqref{eq:cdma_phi_star} & $\pi a_1^2 + 2\pi a_1\sqrt{a_2}$ & $> 1.0$ \\
		直接映射松弛 & $\phi = -\arg(gh)$ & $\pi a_1^2$ & $1.0$ \\
		欧氏投影松弛 & 式~\eqref{eq:cdma_phi_proj} & $\pi \xi_2^2 a_1^2, \; (\xi_2 \approx 0.934)$ & $\approx 0.873$ \\
		\bottomrule
	\end{tabular}
\end{table}

通过对比定理~\ref{the:cdma_gamma}、推论~\ref{cor:cdma_gamma_relax}和推论~\ref{cor:cdma_gamma_proj}，可以分析最优相位配置、直接映射松弛及欧氏投影松弛三种方法的性能。表~\ref{tab:comparison}提取了三种方法平均信噪比表达式中的主导项系数，并以直接映射松弛为基准进行了归一化比较。
三种方法的平均性能满足最优相位配置$\gamma>$直接映射松弛$\gamma^{\mathrm{direct}}>$欧氏投影松弛$\gamma^{\mathrm{proj}}$。
最优相位配置方法通过充分利用洛伦兹约束的固有偏置，保留了直接映射松弛所忽略的交叉项增益，在性能上严格优于后者。特别是在$N_e$较小时，这种相干增益在总能量中占比较大，最优配置方法的优势将更为显著。
而欧氏投影松弛虽在几何上最小化了单元层面的映射误差，却因引入额外的相位失配导致了约$12.8\%$的功率衰减，其有效相干增益反而低于直接映射松弛。
...
前文分析了在完美CSI条件下的最优相位配置及其性能上限。然而，在实际部署中，DMA阵列通常包含数以百计的辐射单元，受限于硬件成本和功耗，射频链路的数量$N_d$远小于辐射单元总数$N$。在这种架构下，基带处理单元通常只能获取每条波导叠加后的等效信道信息，而难以通过传统的导频训练方案单独估计每个辐射单元的信道系数$g_{i,l}h_{i,l}$。若采用开关轮询的方式逐个估计，导频开销将随$N_e$线性增长，导致系统传输效率下降。鉴于此，本节提出一种基于接收信号强度指示的相位对齐算法。由于用户端能够容易地测量下行链路的接收功率并反馈给基站，该方法无需单独估计信道，而是利用RSSI反馈直接优化DMA相位配置，从而最大化接收信噪比。
...
上述分析给出了中间参数$\bm{x}$的最优估计，但实际波束赋形应用中更关注的是相位参数 $\psi$ 的估计精度。
...
\end{align}
代入$x_1 = 2|T_1||T_2| \cos\psi$ 与 $x_2 = 2|T_1||T_2| \sin\psi$，可得均匀采样下的相位估计量$\hat{\psi}$的均方误差为：
\begin{equation}
	\operatorname{MSE}_{\min}(\hat{\psi}) = \frac{\sigma_\epsilon^2}{2 M |T_1|^2 |T_2|^2}.
\end{equation}
...
根据几何关系，有：
\begin{equation}\label{eq:TrefT2*}
...
\end{equation}
利用该能量值对观测矢量进行自干扰剔除，代入式~\eqref{eq:TrefT2*}，可得：
\begin{equation}
