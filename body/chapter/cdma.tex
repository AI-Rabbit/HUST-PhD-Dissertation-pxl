...
为了全面评估所提的最优相位配置方法及基于RSSI的相位对齐算法的性能，本节将通过数值仿真进行验证与分析。本章考虑下行链路通信场景，基站配备的DMA阵列默认包含 $N_d = 10$ 条独立的微带波导，每条波导上集成 $N_e = 40$ 个辐射单元，总辐射单元数量为 $N = 400$。系统工作于典型的微波频段，载波频率设定为 $f_c = 2.6 \text{ GHz}$。

对于DMA内部的波导传输物理特性，设定波导的衰减系数为 $\alpha = 0.5 \text{ Np/m}$，波数为 $\beta = 77.46 \text{ rad/m}$，相应的导波波长为 $\lambda_g = 2\pi/\beta$。为减小超表面单元间的互耦效应并满足弱耦合假设，辐射单元的间距设置为半个导波波长，即 $D = 0.5 \lambda_g$。

在通信信道与信号参数方面，基站的发射功率设定为 $P_t = 30 \text{ dBm}$，单天线用户端的接收噪声功率为 $\sigma^2 = -100 \text{ dBm}$。假设接收用户与基站DMA阵列之间的距离为 $d = 100 \text{ m}$，大尺度衰落采用城市微小区非视距（UMi-NLOS）路径损耗模型，其路损值（单位：dB）计算公式为 $31.24 + 35.3 \log_{10}(d)$。

对于本文提出的基于RSSI的相位对齐算法，在单次测量配置中，每个单元的相位采样点数默认设定为 $M = \textcolor{red}{XX}$，RECD算法的坐标下降迭代次数设定为 $K = \textcolor{red}{XX}$。除非在具体仿真图表中另有说明，本节所有的数值仿真均采用表~\ref{tab:cdma_sim_params}中列出的默认参数，且所有统计结果均通过 $N_{\mathrm{mc}} = \textcolor{red}{XX}$ 次相互独立的蒙特卡洛随机试验取平均得到。

\begin{table}[!htbp]
	\centering
	\caption{DMA波束赋形与相位对齐算法仿真参数表}
	\label{tab:cdma_sim_params}
	\begin{tabular}{cc}
		\toprule
		\textbf{参数} & \textbf{数值} \\
		\midrule
		载波频率 $f_c$ & $2.6 \text{ GHz}$ \\
		波导衰减系数 $\alpha$ & $0.5 \text{ Np/m}$ \\
		波数 $\beta$ & $77.46 \text{ rad/m}$ \\
		辐射单元间距 $D$ & $0.5 \lambda_g$ \\
		波导数量 $N_d$ & $10$ \\
		单条波导辐射单元数 $N_e$ & $40$ \\
		基站至用户距离 $d$ & $100 \text{ m}$ \\
		大尺度衰落路径损耗模型 & UMi-NLOS \\
		基站发射功率 $P_t$ & $30 \text{ dBm}$ \\
		用户端噪声功率 $\sigma^2$ & $-100 \text{ dBm}$ \\
		每单元相位采样点数 $M$ & $\textcolor{red}{XX}$ \\
		RECD算法迭代次数 $K$ & $\textcolor{red}{XX}$ \\
		蒙特卡洛试验次数 $N_{\mathrm{mc}}$ & $\textcolor{red}{XX}$ \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{figure}[!htbp]
	\centering
	%	\includegraphics[width=0.6\linewidth]{cdma/.pdf}
	\caption{xxx}
	\label{fig:}
\end{figure}
