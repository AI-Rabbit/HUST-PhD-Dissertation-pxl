...
\section{引言}\label{ch:cdma_intro}

根据前文的介绍可知，DMA技术通过将包含可调谐超材料单元的平面阵列与传统的波导馈电技术相结合，能够以较少的射频链路驱动大规模天线阵列。硬件成本和功耗方面的优势使其成为 6G 物理层架构演进的关键技术之一。近年来，学术界针对DMA辅助的无线通信系统开展了广泛的研究，涵盖了上下行传输\cite{jiang2022hybrid}、全息波束赋形\cite{altinoklu2025investigation}、近场波束聚焦\cite{gavriilidis2025near}等场景。

然而，现有关于DMA波束赋形的研究在理论模型和算法设计上仍存在一些局限。
首先，DMA 辐射单元的幅相响应由洛伦兹谐振决定，幅度和相位非线性耦合，不能独立调节。现有文献往往将约束理想化为恒定幅度，求得松弛解后再投影回实际可行域。这种近似处理破坏了相位对齐条件，会带来波束增益损失。第二，信号在波导中传播有介质衰减，系统性能受波导衰减和阵列规模双重影响。现有研究大多只做数值仿真，缺乏考虑波导物理特性的闭式性能分析，难以从理论上指导实际DMA系统的天线规模配置。最后，传统波束赋形高度依赖精确的CSI，对于数千个单元的DMA来说，估计CSI的庞大导频开销在实际工程中是难以接受的。
...
本章针对DMA波束赋形中的洛伦兹幅相耦合约束、波导衰减效应以及大规模阵列下 CSI 获取开销大的难题，开展了连续相移条件下的性能分析与低开销算法设计。理论层面，通过解耦洛伦兹约束推导了最优连续相位的闭式解，并建立推导了系统平均信噪比、中断概率与遍历容量的解析框架。分析得出，系统波束增益随单波导单元数 $N_e$ 增加呈现先升后降的规律，在弱衰减近似下，最佳单元数 $N_e^\star \approx 1.256/(\alpha D)$。同时，渐近分析证明了所提最优配置在相干增益上优于松弛方法，相较于直接映射松弛，相对增益可达$20\%$至$70\%$。算法层面，提出了无需显式 CSI 的盲波束对齐方案。通过建立相位估计的线性回归模型，从理论上证明了均匀采样能消除参数估计耦合。仿真结果验证了理论分析的准确性，表明在采样点数 $M \ge 4$ 时即可获得各向同性且最小的均方误差。
相较于随机搜索，所提RECD与NI-REPA算法收敛更快，特别是利用全局参考剔除自干扰的NI-REPA算法，在 40 dBm 发射功率下仅需一轮测量即可达到  $95\%$ 以上的归一化接收功率。
