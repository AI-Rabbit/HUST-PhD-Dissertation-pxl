\chapter{离散相移DMA波束赋形性能分析与角域分划算法设计}\label{ch:ddma}


\section{引言}\label{sec:ddma_intro}

前一章探讨了理想连续相移条件下动态超表面天线的波束赋形机制及理论性能边界。然而，在实际的原型系统中，通常采用 PIN 二极管或变容二极管作为超材料单元的可调谐元件，受控制电路复杂度和 DAC 量化精度限制，单元相移状态往往是有限且离散的。受工艺公差和器件物理特性影响，这些离散相移还可能呈现非均匀分布的特性。因此，有必要进一步研究离散相位约束下的 DMA 波束赋形理论与算法。

近年来，已有一些研究探讨了可重构天线系统离散相移约束下的波束赋形设计，但大多数集中于RIS系统。例如，文献\lcite{wu2020beamforming}分析了在完美CSI下，离散相移约束对 RIS 辅助系统通信性能的影响。然而，虽然 DMA 与 RIS 均利用相移调控电磁波，但两者不同的物理结构导致了不同的信号传播模型与约束条件，直接套用RIS相关理论结论会导致 DMA 架构下的性能评估失真。在算法设计方面，文献\lcite{zhang2021optimal}针对具有均匀相位分布的 RIS 提出了一种最优的 1 比特波束赋形算法。但由于未结合 DMA 的洛伦兹约束，将其直接应用于 DMA 系统会产生相位映射误差。此外，现有研究也未能充分涵盖高分辨率量化以及任意非均匀离散相移的场景。

针对上述难题，本章深入分析了离散相移约束对 DMA 系统波束赋形性能的影响，并提出了一种具备多项式时间复杂度的全局最优离散波束赋形算法。本章的主要研究内容归纳如下：

\begin{enumerate}
	\item 建立了任意离散相移条件下的 DMA 波束赋形理论分析框架。通过推导平均信噪比定量刻画了离散相移带来的性能损失。在此基础上证明了在给定的硬件移相能力约束下，均匀量化策略能够最大化系统波束赋形增益。
	
	\item 提出了基于多路归并的角域排序分划搜索算法。该算法通过引入辅助变量与几何投影，将多维离散组合优化问题转化为一维角域上的单变量搜索问题，随后设计了多路归并排序策略，实现了在多项式级别复杂度下对离散量化约束全局最优解的求解。
	
	\item 进一步探讨了均匀量化场景下 ADSPS 算法的性能界限。推导了该算法在不同量化分辨率下的性能下界，从理论上证明了其在低精度量化场景中优于CPP算法。最后，通过数值仿真验证了 ADSPS 算法在各类离散约束条件下的有效性。
\end{enumerate}

本章剩余部分组织如下：第~\ref{sec:ddma_system_model}节给出考虑任意离散相移约束和莱斯衰落信道的 DMA 下行通信系统模型；第~\ref{sec:ddma_cpp}节推导任意离散相移下的平均信噪比闭式解，并分析离散相位分布规律对系统性能的影响；第~\ref{sec:ddma_adsps}节详细介绍所提 ADSPS 算法的几何转化、快速排序机制及在均匀量化下的性能下界；第~\ref{sec:ddma_sim}节通过数值仿真验证理论推导和算法效果；第~\ref{sec:ddma_conclusion}节对本章研究内容进行总结。


\section{系统模型}\label{sec:ddma_system_model}
...
\section{最近点投影算法与离散性能分析}\label{sec:ddma_cpp}
...
本小节将推导任意离散相移约束下，采用CPP算法时DMA系统的平均信噪比闭式解。定义归一化瞬时信道分量 $g_{i,l}' = g_{i,l} / \sqrt{\rho}$。根据莱斯衰落模型，其包络 $|g_{i,l}'|$ 服从二阶矩 $\mathbb{E}[|g_{i,l}'|^2] = 1$ 且参数为 $\kappa$ 的莱斯分布。该包络的数学期望 $\mu_g \triangleq \mathbb{E}[|g_{i,l}'|]$ 可由第一类修正贝塞尔函数 $I_0(\cdot)$ 和 $I_1(\cdot)$ 严格表示为：
\begin{equation}\label{eq:ddma_mug}
	\mu_g = \sqrt{\frac{\pi}{4(\kappa+1)}} e^{-\frac{\kappa}{2}} \left[ (1+\kappa) I_0\left(\frac{\kappa}{2}\right) + \kappa I_1\left(\frac{\kappa}{2}\right) \right].
\end{equation}

根据式~\eqref{eq:cdma_gamma}定义的平均信噪比，代入全信道模型及CPP算法得到的离散权重系数$\bm Q^{\text{CPP}}$，由于所有波导具有相同的统计特性，系统平均信噪比 $\gamma^{\text{CPP}}$ 可表示为：
\begin{equation}\label{eq:ddma_gamma}
	\gamma^{\text{CPP}} = \frac{P_t \rho N_d}{\sigma^2} \mathbb{E}\left(\left| \sum_{l=1}^{N_{e}} g_{i,l}' h_{i, l} q_{i,l}^{\text{CPP}}\right|^{2}\right).
\end{equation}

定义CPP算法引入的量化相位误差为 $\varphi_{i,l} = \phi_{i,l}^{\text{CPP}} - \phi_{i,l}^\star$。将 $q_{i,l}^{\text{CPP}} = (j+e^{j(\phi_{i,l}^\star + \varphi_{i,l})})/2$ 以及连续最优相位 $\phi_{i,l}^\star$ 的表达式代入，上述期望项可化简为：
\begin{equation}\label{eq:ddma_E_total}
	\mathbb{E}\left(\left| \sum_{l=1}^{N_{e}} g_{i,l}' h_{i, l} q_{i,l}^{\text{CPP}}\right|^{2}\right) = \frac{1}{4} \mathbb{E}\left( \left| |\Lambda_i| + \sum_{l=1}^{N_e} |g_{i,l}' h_{i,l}| e^{j\varphi_{i,l}} \right|^2 \right),
\end{equation}
其中 $\Lambda_i = j \sum_{l=1}^{N_{e}} g_{i,l}' h_{i,l}$ 为与量化误差无关的固有信道常数项。对模平方进行展开，得到如下三项之和：

\begin{equation}\label{eq:ddma_E_terms}
	\medmath{
		\mathbb{E}\left( | \cdots |^2 \right) = \frac{1}{4} \mathbb{E}
		\bigg( \underbrace{|\Lambda_i|^2 + \sum_{l=1}^{N_e} |g_{i,l}' h_{i,l}|^2}_{A_1} + \underbrace{2|\Lambda_i| \sum_{l=1}^{N_e} |g_{i,l}' h_{i,l}| \cos \varphi_{i,l}}_{A_2} + \underbrace{\sum_{l \ne \lambda} |g_{i,l}' h_{i,l}| |g_{i,\lambda}' h_{i,\lambda}| e^{j(\varphi_{i,l} - \varphi_{i,\lambda})}}_{A_3} \bigg).
	}
\end{equation}

下面逐项计算 $A_1, A_2, A_3$ 的期望值。
对于项 $A_1$，由于不同单元的信道相互独立且均值为零，交叉项期望为零。结合 $\mathbb{E}[|g_{i,l}'|^2] = 1$ 以及波导衰减特性，可得：
\begin{equation}
	\mathbb{E}(A_1) = 2 \sum_{l=1}^{N_e} \mathbb{E}[|g_{i,l}'|^2] |h_{i,l}|^2 = 2 \sum_{l=1}^{N_e} |h_{i,l}|^2 = \frac{2}{N_e} \frac{1 - e^{-2\alpha D N_e}}{1 - e^{-2\alpha D}}.
\end{equation}

对于项 $A_2$，当单元数量 $N_e$ 较大时，根据中心极限定理，固有项 $\Lambda_i$ 近似服从循环对称复高斯分布，其模值 $|\Lambda_i|$ 服从瑞利分布。其均值为：
\begin{equation}
...
\end{equation}
结合包络期望 $\mathbb{E}[|g_{i,l}'|] = \mu_g$，代入 $A_2$ 的定义式，得到：
\begin{equation}
	\mathbb{E}(A_2) = \frac{\sqrt{\pi}}{N_e} \mu_g \sqrt{\frac{1 - e^{-2\alpha D N_e}}{1 - e^{-2\alpha D}}} \frac{1 - e^{-\alpha D N_e}}{1 - e^{-\alpha D}} \mathbb{E}(\cos \varphi_{i,l}).
\end{equation}

最后计算项 $A_3$ 的期望。由于不同单元的包络与量化误差均相互独立，且 $\mathbb{E}(e^{j\varphi_{i,l}}) = \mathbb{E}(\cos \varphi_{i,l})$，有：
\begin{equation}
	\mathbb{E}(A_3) = \frac{\mu_g^2}{N_e} \left[ \left(\frac{1 - e^{-\alpha D N_e}}{1 - e^{-\alpha D}}\right)^2 - \frac{1 - e^{-2\alpha D N_e}}{1 - e^{-2\alpha D}} \right]\cdot [\mathbb{E}(\cos \varphi_{i,l})]^2.
\end{equation}
...
综合上述各项期望的推导结果，代入式~\eqref{eq:ddma_gamma}，总结为如下定理。
...
\begin{theorem}\label{the:ddma_gamma_cpp}
	...
	\begin{equation}\label{eq:ddma_gamma_cpp_closed}
		\gamma^{\text{CPP}} = \frac{P_t \rho N_d}{4\sigma^2 N_e} \left( 2a_2 + \mu_g \sqrt{\frac{a_2 a_1^2}{\pi}} \sum_{m=1}^M \sin \frac{\delta_m}{2} + \mu_g^2 \frac{a_1^2 - a_2}{\pi^2} \left( \sum_{m=1}^M \sin \frac{\delta_m}{2} \right)^2 \right),
	\end{equation}
	其中，$\mu_g$ 为式~\eqref{eq:ddma_mug}给出的莱斯信道归一化包络期望；$\delta_m = \Phi_{m+1}-\Phi_m$（约定 $\Phi_{M+1} = \Phi_1 + 2\pi$）；$a_1 = \frac{1 - e^{-\alpha D N_e}}{1 - e^{-\alpha D}}$ 且 $a_2 = \frac{1 - e^{-2\alpha D N_e}}{1 - e^{-2\alpha D}}$ 为波导衰减相关的系数。
\end{theorem}
...
\end{equation}
其中$M=2$时，求和上限为负数，求和项取0，仅由第二项得出结果。式~\eqref{eq:ddma_ESrandom_closed_floor}表征了在相位配置完全随机、缺乏任何优化设计时的系统平均性能，构成了DMA系统性能的下界基准。

进一步地，根据定理~\ref{the:ddma_gamma_cpp}，为了得到随机相移条件下的系统平均信噪比，还需要推导相位性能因子的二阶矩 $\mathbb{E}[\mathcal{S}_{\text{rand}}^2]$。将其平方项展开，可分解为自项与交叉项之和：
\begin{equation}
	\mathbb{E}[\mathcal{S}_{\text{rand}}^2] = \mathbb{E}\left[ \left( \sum_{m=1}^M \sin\frac{\delta_m}{2} \right)^2 \right] 
	= M \underbrace{\mathbb{E}\left[\sin^2\frac{\delta_1}{2}\right]}_{E_{\text{auto}}} + M(M-1) \underbrace{\mathbb{E}\left[\sin\frac{\delta_1}{2} \sin\frac{\delta_2}{2}\right]}_{E_{\text{cross}}}.
\end{equation}

首先计算自项 $E_{\text{auto}}$。利用二倍角公式以及前文得出的 $f_\delta(x)$，并通过换元 $u = 1 - \frac{x}{2\pi}$，可得：
\begin{align}
	E_{\text{auto}} &= \int_0^{2\pi} \frac{1-\cos x}{2} \frac{M-1}{2\pi}\left(1-\frac{x}{2\pi}\right)^{M-2} \mathrm{d}x \nonumber \\
	&= \frac{M-1}{2} \int_0^1 u^{M-2} \left( 1 - \cos(2\pi - 2\pi u) \right) \mathrm{d}u \nonumber \\
	&= \frac{1}{2} - \frac{M-1}{2} J_{M-2}, \label{eq:ddma_E_auto}
\end{align}
其中，定义辅助积分序列 $J_n = \int_0^1 u^n \cos(2\pi u) \mathrm{d}u$。与前文推导$I_n$类似，通过连续两次分部积分，容易得出该序列满足二阶递推关系：
\begin{equation}
	J_n = \frac{n}{4\pi^2} - \frac{n(n-1)}{4\pi^2} J_{n-2},
\end{equation}
其中初值为 $J_0 = 0, J_1 = 0$。

接着计算交叉项 $E_{\text{cross}}$。假设量化阶数 $M \ge 3$，在 $2\pi$ 周期内随机划分 $M$ 个区间，任意两个不同间隔 $\delta_1, \delta_2$ 的联合概率密度函数为：
\begin{equation}
	f_{\delta_1, \delta_2}(x, y) = \frac{(M-1)(M-2)}{(2\pi)^2} \left(1 - \frac{x+y}{2\pi}\right)^{M-3},
\end{equation}
其定义域满足 $x \ge 0, y \ge 0$ 且 $x+y \le 2\pi$。引入变量替换 $u = \frac{x}{2\pi}, v = \frac{y}{2\pi}$，并利用积化和差公式 $\sin\frac{x}{2}\sin\frac{y}{2} = \frac{1}{2}(\cos\frac{x-y}{2} - \cos\frac{x+y}{2})$，积分可化为：
\begin{equation}
	E_{\text{cross}} = \frac{(M-1)(M-2)}{2} \iint_{\substack{u \ge 0, v \ge 0 \\ u+v \le 1}} \left[ \cos(\pi(u-v)) - \cos(\pi(u+v)) \right] (1-u-v)^{M-3} \mathrm{d}u \mathrm{d}v.
\end{equation}

为简化二重积分，进行雅可比线性变换，令 $w = u+v \in [0,1]$，$z = u-v \in [-w, w]$，对应的雅可比行列式绝对值为 $1/2$。先对 $z$ 积分，再对 $w$ 积分：
\begin{align}
	E_{\text{cross}} &= \frac{(M-1)(M-2)}{4} \int_0^1 (1-w)^{M-3} \left[ \int_{-w}^w \big( \cos(\pi z) - \cos(\pi w) \big) \mathrm{d}z \right] \mathrm{d}w \nonumber \\
	&= \frac{(M-1)(M-2)}{2} \int_0^1 (1-w)^{M-3} \left[ \frac{1}{\pi}\sin(\pi w) - w\cos(\pi w) \right] \mathrm{d}w.
\end{align}
再次令 $t = 1-w$，上式转换为：
\begin{equation}
	E_{\text{cross}} = \frac{M-1}{2} \int_0^1 (M-2)t^{M-3} \left[ \frac{1}{\pi}\sin(\pi t) + (1-t)\cos(\pi t) \right] \mathrm{d}t.
\end{equation}
观察被积函数可以发现，$(M-2)t^{M-3}$ 恰为 $t^{M-2}$ 的导数，因此可采用分部积分法。利用边界条件消去常数项后，积分转化为仅含 $\sin(\pi t)$ 的形式：
\begin{align}
	E_{\text{cross}} &= \frac{M-1}{2} \left[ 0 - \int_0^1 t^{M-2} \big( -\pi(1-t)\sin(\pi t) \big) \mathrm{d}t \right] \nonumber \\
	&= \frac{\pi(M-1)}{2} \int_0^1 \left( t^{M-2} - t^{M-1} \right) \sin(\pi t) \mathrm{d}t \nonumber \\
	&= \frac{\pi(M-1)}{2} \left( I_{M-2} - I_{M-1} \right), \label{eq:ddma_E_cross}
\end{align}
其中 $I_n$ 是式~\eqref{eq:ddma_In_recursion} 中的正弦积分序列。

综合式~\eqref{eq:ddma_E_auto}与式~\eqref{eq:ddma_E_cross}，$\mathbb{E}[\mathcal{S}_{\text{rand}}^2]$ 最终表示为：
\begin{equation}\label{eq:ddma_E_S2_closed}
	\mathbb{E}[\mathcal{S}_{\text{rand}}^2] = \frac{M}{2} - \frac{M(M-1)}{2} J_{M-2} + \frac{\pi M(M-1)^2}{2} (I_{M-2} - I_{M-1}).
\end{equation}
上式对于 $M=2$ 的 情况同样适用。将式~\eqref{eq:ddma_ESrandom_closed_floor}与式~\eqref{eq:ddma_E_S2_closed}代入定理~\ref{the:ddma_gamma_cpp}，可得到随机相位约束下 DMA 系统的平均信噪比基准。为了获得最佳的波束赋形增益，需要优化相位集合 $\bm \Phi_M$ 的分布，以最大化因子 $\mathcal{S}$。该优化问题可表述为在间隔之和满足一定的约束条件下，寻找最优的间隔序列 $\{\delta_m\}$：
...
由于全周期均匀量化是最通用且性能最优的方案，在此场景下，与第\ref{ch:cdma}章类似，将所提CPP算法与直接映射松弛下的量化方法进行对比。
结合定理~\ref{the:ddma_gamma_cpp}、定理~\ref{the:ddma_opt_phase}与推论~\ref{cor:cdma_gamma_relax}，得到均匀量化下CPP算法与松弛方法的平均信噪比表达式：
\begin{align}
	\gamma^{\text{CPP}} &= \frac{P_t \rho N_d}{4\sigma^2 N_e} \left( 2a_2 + \mu_g M\sqrt{\frac{a_2 a_1^2}{\pi}} \sin \frac{\pi}{M} + \mu_g^2 \frac{M^2(a_1^2-a_2)}{\pi^2} \sin^2 \frac{\pi}{M} \right), \label{eq:ddma_gamma_cpp_uni}\\
	\gamma^{\text{RLX}} &= \frac{P_t \rho N_d}{4\sigma^2 N_e} \left( 2a_2 + \mu_g^2 \frac{M^2(a_1^2-a_2)}{\pi^2} \sin^2 \frac{\pi}{M} \right). \label{eq:ddma_gamma_rlx_uni}
\end{align}
...
\section{数值仿真分析}\label{sec:ddma_sim}

\subsection{仿真参数设置}

本节通过数值仿真来验证本章提出的CPP算法与ADSPS算法在离散相移约束下的波束赋形性能，并评估不同相位分布策略对系统平均信噪比的具体影响。仍然考虑下行链路通信场景，基站端的DMA阵列默认包含 $N_d = 10$ 条平行微带波导，每条波导集成 $N_e = 40$ 个辐射单元，总计 $N = 400$ 个天线单元。波导相关的物理参数与上一章保持一致：衰减系数 $\alpha = 0.5 \text{ Np/m}$，波数 $\beta = 77.46 \text{ rad/m}$，单元间距设为半个导波波长。本章重点关注受限硬件条件下的系统性能，量化精度的设定将在对应的仿真中具体说明。
系统载波频率设为 $f_c = 2.6 \text{ GHz}$。基站发射功率 $P_t = 30 \text{ dBm}$，接收端噪声功率 $\sigma^2 = -80 \text{ dBm}$，收发两端距离 $d = 100 \text{ m}$。
本章小尺度衰落采用莱斯衰落模型，莱斯因子 $\kappa = 10$；大尺度衰落采用3GPP城市宏小区视距模型，其路径损耗（单位：dB）的计算公式为：
\begin{equation}
	\mathrm{PL} = 36.3 + 22 \log_{10}(d).
\end{equation}
如果没有特别说明，后续所有的数值仿真都将基于上述参数开展。各图表中的统计曲线均由 $N_{\mathrm{mc}} = 2000$ 次独立的蒙特卡洛随机试验取平均得到。

\subsection{离散相移统计特性理论验证}

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figSNRvsNe.pdf}
	\caption{系统平均信噪比随波导单元数 $N_e$ 的变化趋势及理论验证}
	\label{fig:ddma_figSNRvsNe}
\end{figure}

首先验证定理~\ref{the:ddma_gamma_cpp}所推导的平均信噪比的准确性。图~\ref{fig:ddma_figSNRvsNe}展示了系统平均信噪比随单条波导单元数 $N_e$ 的变化趋势。为了覆盖定理的适用范围，仿真设定了五种不同的相位分布场景：理想连续相移、高/低精度均匀相移、特定的非均匀相移以及作为基准的1 bit随机相移。从图中可以看出，在所有相位配置下，理论实线与仿真散点均实现了精确吻合，证明了莱斯信道下离散相移DMA系统平均信噪比解析推导的正确性。对比不同曲线间的相对位置可以发现，在相同的量化阶数下，均匀相移配置的信噪比优于非均匀相移。此外，所有曲线均在同一位置达到增益峰值，这表明使得系统性能最大化的最优天线单元数与相位量化方式无关，仅由波导衰减决定。

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figQuantizationLossPhase.pdf}
	\caption{系统归一化性能随硬件移相能力 $C$ 的演化规律}
	\label{fig:ddma_figQuantizationLossPhase}
\end{figure}

接着，考虑定理~\ref{the:ddma_opt_phase}中的移相能力受限场景，图~\ref{fig:ddma_figQuantizationLossPhase}绘制了归一化性能随系统最大移相能力 $C$ 的演化规律。图中垂直虚线标定了针对不同量化阶数 $M$ 的临界阈值 $C_{\text{th}} = \frac{2\pi(M-1)}{M}$。从图中可以观察到，各条曲线初期均随 $C$ 的拓宽而单调递增；当移相能力跨过对应的理论阈值后，各曲线达到饱和，准确收敛于全周期均匀量化下的性能上界。更为关键的是，当移相能力严重受限时，采用高精度量化相比于低精度量化所带来的额外增益相当微小。因此，在此类场景下，系统设计的重心应优先放在放宽器件的物理移相范围上，而非盲目追求高分辨率的相位控制。

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figQuantizationLoss.pdf}
	\caption{均匀量化下不同算法的归一化性能随波导单元数 $N_e$ 的变化对比}
	\label{fig:ddma_figQuantizationLoss}
\end{figure}

进一步地，为了对比本章所提算法与传统直接映射松弛算法在处理离散约束时的性能，图~\ref{fig:ddma_figQuantizationLoss}给出了均匀量化场景下系统归一化性能随波导单元数 $N_e$ 的演变规律。图中结果直观地印证了式~\eqref{eq:ddma_gamma_cpp_uni}与式~\eqref{eq:ddma_gamma_rlx_uni}的对比结论，CPP算法的量化损失在任何量化精度下均小于RLX算法，且该优势在低分辨率量化时尤为显著。从具体数值来看，随着阵列规模 $N_e$ 的扩展，系统的量化损失迅速收敛至稳定状态。在1比特量化条件下，传统RLX算法的量化损失约为 $4.7 \text{ dB}$；而CPP算法的损失约为 $3.4 \text{ dB}$，比传统方法损失低 $1.3 \text{ dB}$。当精度提升至 $M=4$ 时，CPP算法的量化损失已降至约 $0.8 \text{ dB}$；而对于 $M=8$ 的高精度配置，CPP算法的量化损失不足 $0.2 \text{ dB}$。


\subsection{角域排序分划搜索算法性能验证}

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figExhaustive.pdf}
	\caption{不同相移配置下ADSPS算法与穷举搜索及CPP算法的性能对比}
	\label{fig:ddma_figExhaustive}
\end{figure}

本小节通过数值仿真评估所提ADSPS算法的波束赋形性能。图~\ref{fig:ddma_figExhaustive}对比了ADSPS算法、CPP算法以及作为最优上界的穷举搜索法的系统平均信噪比。由于穷举搜索的计算复杂度随天线单元数呈指数级膨胀，仿真的单条波导单元数设定在 $N_e \le 14$ 的范围内进行。从图中可以看出，无论系统配置为均匀相移还是非均匀受限相移，ADSPS算法所绘制的实线均与穷举搜索获取的散点严密重合，证明了该算法能得到任意离散约束下的全局最优解。
此外，通过与CPP算法对比可以发现，算法间的性能差距与量化相移数关系密切。在较高量化精度时，CPP算法的性能与ADSPS算法接近；但当硬件为1比特量化或处于非均匀受限状态时，CPP算法的平均SNR比ADSPS算法低约 $0.3 \text{ dB}$。

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figNormPerfvsNe_ADSPS.pdf}
	\caption{不同算法在三种离散约束下的归一化性能随 $N_e$ 的变化曲线}
	\label{fig:ddma_figNormPerfvsNe_ADSPS}
\end{figure}

将仿真规模扩展至大尺度阵列，图~\ref{fig:ddma_figNormPerfvsNe_ADSPS}对比了ADSPS与CPP算法在三种离散约束下的归一化性能变化曲线，以评估两者的量化损失渐近特性。随着天线单元数 $N_e$ 的增加，各曲线在经历初期的快速下降后，均迅速收敛至稳定的量化损失下界。在精度相对较高的 $M=4$ 均匀量化配置下，ADSPS与CPP算法的性能差距较小，最终量化损失分别稳定在 $0.7 \text{ dB}$ 和 $0.8 \text{ dB}$。当采用1比特量化时，ADSPS算法则展现出更强的鲁棒性。具体而言，在均匀量化场景下，ADSPS算法的量化损失稳定在 $3.1 \text{ dB}$ 左右，相比CPP算法降低了近 $0.3 \text{ dB}$；在非均匀受限场景下，CPP算法的量化损失大于 $4.4 \text{ dB}$，而ADSPS算法则维持在 $4.1 \text{ dB}$ 附近。上述对比表明，在低分辨率或非均匀相位约束的大规模DMA系统中，ADSPS算法能够带来稳定可观的性能提升。

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figCDF.pdf}
	\caption{不同相移配置下各算法信噪比的累积分布函数}
	\label{fig:ddma_figCDF}
\end{figure}

图~\ref{fig:ddma_figCDF}展示了固定波导单元数 $N_e=40$ 时各算法输出信噪比的累积分布函数。在 $M=4$ 的较高精度配置下，ADSPS算法与CPP算法的性能表现接近，这与前面的仿真结果一致。当系统采用1比特量化时，算法间的性能差异开始显现。观察图中的CDF轨迹可知，同一相移配置下ADSPS与CPP算法的CDF曲线并不平行：两者在高信噪比区域距离较近，但在低信噪比区域的横向间距明显拉大。进一步对比 $M=2$ 时不同的量化分布可以发现，非均匀相移约束对低信噪比区域的拖累更为严重。此外，通过观察执行了1000次采样的RS算法曲线可以得出两个结论：一是单纯增加量化精度对随机搜索策略的性能几乎没有提升；二是在引入非均匀相移后，该算法在低信噪比区间的性能表现呈现出断崖式恶化。

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.6\linewidth]{ddma/ddma_figNormPerfvsPhase_ADSPS.pdf}
	\caption{ADSPS算法归一化性能随移相能力 $C$ 的变化曲线}
	\label{fig:ddma_figNormPerfvsPhase_ADSPS}
\end{figure}

最后探讨硬件移相能力 $C$ 对ADSPS算法相对性能提升的影响。图~\ref{fig:ddma_figNormPerfvsPhase_ADSPS}以 $M=2$ 配置下的CPP算法为基准，展示了不同目标量化阶数下ADSPS算法的归一化性能变化。观察图中的演化轨迹可知，ADSPS算法在全区间内均实现了严格的正向增益。当可调相位区间偏离均匀分布较远，即处于 $C < \pi$ 的严重受限状态时，该相对增益比均匀相移时更高。以 $M=2$ 的场景为例，算法增益在 $C \approx \pi/4$ 附近达到了近 $0.5 \text{ dB}$ 的峰值；当 $C$ 跨过 $\pi$ 后，性能增益逐渐回落并稳定在 $0.3 \text{ dB}$ 左右。$M=4$ 场景下的曲线亦呈现出相似的演变特征。上述结果表明，底层硬件的物理移相能力越受限、离散相位分布越不均匀，ADSPS算法能够带来的性能增益就越大。


\section{本章小结}\label{sec:ddma_conclusion}

本章针对实际 DMA 硬件面临的低分辨率离散、非均匀以及移相能力受限等约束，在莱斯衰落信道下开展了离散波束赋形性能分析与全局优化算法设计。
理论方面，基于CPP算法推导了任意离散相移下平均信噪比的闭式解。理论分析表明，当移相能力 $C \ge 2\pi(M-1)/M$ 时，全周期均匀分布是最优量化策略。此外，推导揭示了CPP算法在1-bit条件下存在完全失效的风险。
算法层面，提出了一种具备全局最优性的ADSPS算法。通过引入辅助变量将离散组合优化转化为一维角域搜索，配合多路归并策略，将计算复杂度从穷举的指数级 $\mathcal{O}(N_d M^{N_e})$ 降至多项式级，均匀量化下可进一步降低至 $\mathcal{O}(N_d(N_e \log N_e + N_e))$。
理论上，ADSPS 算法在$M$阶均匀量化下能保证$( \frac{M}{\pi} \sin \frac{\pi}{M} )^2$ 的下界，即使在 1-bit 这种极端条件下也能至少获取连续最优解 $25\%$ 的波束赋形增益。
数值仿真验证了理论推导的准确性。在 1 比特均匀量化的大规模阵列中，ADSPS 算法的量化损失稳定在 3.1 dB 左右，相比传统直接映射松弛算法和 CPP 算法分别降低了 1.6 dB 和 0.3 dB。在相移能力为$\pi/4$的严重受限场景下，ADSPS 算法较 CPP 算法可获得近 0.5 dB 的额外性能增益，验证了其在低分辨率受限条件下的鲁棒性。
