...
值得指出的是，在推导交叉项 $E_{\text{cross}}$ 时引入了 $M \ge 3$ 的假设以确保二维联合概率密度的存在性。然而，该闭式解在代数结构上具有完备性，自然兼容 $M=2$ 的边界情况。具体而言，当 $M=2$（即 1 比特量化）时，两个相位间隔满足严格的线性约束 $\delta_2 = 2\pi - \delta_1$。此时目标因子退化为 $\mathcal{S}_{\text{rand}} = 2\sin(\delta_1/2)$，利用一维均匀分布极易求得其真实二阶矩为 $\mathbb{E}[\mathcal{S}_{\text{rand}}^2] = 2$。另一方面，若将 $M=2$ 代入式~\eqref{eq:ddma_E_S2_closed}，结合初始积分值 $J_0=0$、$I_0=2/\pi$ 及 $I_1=1/\pi$，计算结果同样严格为 $2$。这证明了式~\eqref{eq:ddma_E_S2_closed} 所给出的闭式解对任意离散阶数 $M \ge 2$ 均普遍成立。

---


将式~\eqref{eq:ddma_ESrandom_closed_floor}与式~\eqref{eq:ddma_E_S2_closed}代入定理~\ref{the:ddma_gamma_cpp}，可得到随机相位约束下 DMA 系统的平均信噪比基准。为了获得最佳的波束赋形增益，需要优化相位集合 $\bm \Phi_M$ 的分布，以最大化因子 $\mathcal{S}$。该优化问题可表述为在间隔之和满足一定的约束条件下，寻找最优的间隔序列 $\{\delta_m\}$：
