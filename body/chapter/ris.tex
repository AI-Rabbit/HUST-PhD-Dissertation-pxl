...
在阵列控制电路方面，RIS 面板的后端集成了一套基于现场可编程逻辑门阵列（FPGA）的主控系统。FPGA主控板连接了 28 个 8 位串入并出移位寄存器来生成并行的离散逻辑状态，随后通过电平转换器调整输出电压幅值，使得反射单元在 5.8 GHz 工作频率下能够产生约 $180^\circ$ 的相位差。
...
基于上述最优化算法的底层支撑，本章研制的 RIS 原型系统具备了应对更为严苛且复杂的真实商用部署环境的条件。在下一小节中，我们将该系统推向真实的商用 5G 网络，进一步评估其在现网覆盖增强中的实际价值与面临的工程挑战。
...
为了评估我们的RIS系统在商用网络下的性能，我们在一处大型工业园区进行了一项实验。图\ref{fig:ris_outdoor_map}展示了测试环境的照片。基站位于通道入口100米外。手机与RIS相距1.5米，并在法线方向上呈45度角。实验期间，我们使用一款信号测试软件来监测来自基站的信号接收情况并记录其强度。

我们首先在不部署RIS的情况下分析了软件上的小区列表，发现当用户设备在室内时，基站小区不在列表中。接着，我们将RIS放置在通道入口处，天线侧朝向基站。此时，用户设备能够解码目标小区的物理小区标识。然而，其参考信号接收功率低于-90 dBm。我们观察到RSRP随时间波动约2 dB，可能是由于树木、风等环境因素的干扰。这种波动对GFB算法的性能产生了不利影响。

第xxx节提出的TFS码本能够缓解这个问题。它使我们能够根据反射角和相应的导向矢量来确定码字。我们可以测量这些角度并反馈给控制器。或者，我们可以使用TSC算法，按预定义的角度序列依次生成码本，并找到具有最高RSRP的码字。经过多次测试和平均后，我们发现，与RIS关闭时相比，RIS开启时室内信号强度提高了4至7 dB。实验结果比之前的室内实验大约差5 dB。

为了找出这种差异的原因，我们进行了一项对比实验。我们使用一个射频信号源和一台频谱分析仪分别作为发射机和接收机。发射机位于距离RIS 10米处，沿着基站与RIS中心的连线放置，而接收机则位于移动用户原来的位置。我们分别使用全向天线和定向天线进行了实验。定向天线产生了与先前测试相似的增益，约为12 dB。然而，全向天线仅提供了4 dB的增益。这个结果与实际基站到用户设备的场景一致，并且对在实际全向天线环境中提升增益提出了挑战。初步测试结果表明，RIS能够根据用户位置调整信号波束，并在商用网络覆盖不佳的区域改善信号强度和通信质量。
...
\begin{figure}[!htbp] 
	\centering
	\includegraphics[width=0.8\linewidth]{ris/ris_outdoor_map.pdf}
	\caption{xxx}
