...
各算法的实测性能对比数据详见表~\ref{tab:ris_algorithm_comparison}。从表中可以看出，ASG算法展现出了最优异的性能，在所有测试角度下均取得了最高增益，平均增益为 11.9 dB，相较于 DFT 算法与 GFB 算法获得了超过 1 dB 的提升。RS算法的平均增益仅为 4.8 dB，在所有方案中表现最差。这一实测结果与第~\ref{sec:ris_sim}节的数值仿真一致。
...
本章围绕 RIS 实际部署中级联信道估计开销大、优化影响传输等工程问题，研究了基于RSSI反馈的盲波束赋形算法，并研制大规模RIS原型系统，在多类真实场景中开展实测。在算法方面，利用UPA阵列的导向矢量结构特征，提出了GFB算法对搜索空间进行降维，将单次寻优的复杂度降低至$\mathcal{O}(N_z + N_y)$。ASG算法则通过引入动态步长机制，迭代初期的搜索范围较大，后期则转为精细化搜索。这使得ASG算法在多径环境下的鲁棒性更好。仿真结果表明，在256单元、1-bit的RIS配置下，ASG 算法可将信噪比提升至约 46 dB，接近理想 CSI 的性能上界。在不同莱斯因子与量化比特数的条件下，GFB和ASG算法的平均性能均优于CSM算法。
工程实现上，本章研制了工作于 5.8 GHz 和 2.6 GHz 的两套 1-bit RIS 低功耗原型系统。VNA 实测 S 参数表明 RIS 信道在幅相两维度上满足互易性；微波暗室方向图显示，$30^\circ$ 至 $60^\circ$ 范围内波束增益的波动小于 1 dB，各个角度下的副瓣高度平均低于主瓣 9.8 dB，波束调控精度符合设计要求。
在现场测试中，两套RIS系统展现出较强的补盲效果。在室内穿墙NLoS场景下，RIS较同尺寸铜板取得 26 dB 的接收功率增益；在走廊盲区覆盖场景中，根据RIS面板放置角度的不同，实现了6 dB 至 20 dB的增益；在办公区边缘盲区场景中，三种部署位置下 RIS 平均增益为 15.5 dB 至 22 dB。
在各算法性能的横向对比测试中，ASG 算法的平均增益为 11.9 dB，优于 DFT 算法和 GFB 算法，这与仿真结果一致。
最后，在商用5G现网实测中，部署RIS使得室内终端的RSSI提升了 4 至 5 dB，对比实验还表明，宽波束基站与用户全向天线引入的多径效应是现网增益低于实验室测试结果的主要原因。
