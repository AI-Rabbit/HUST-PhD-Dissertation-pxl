...
\section{引言}\label{sec:ris_intro}

前几章主要研究了在基站发射端调控天线电磁响应或动态改变天线物理位置来提升通信性能的技术。然而，无线信号在自由空间中传输时，仍会遇到遮挡造成的深度衰落，仅靠发射端的技术改进不足以实现全域覆盖，还需要主动重塑电磁波的传播环境。可重构智能表面成本低、功耗低且易于部署，能够定制无线传播环境，构建高质量的 LoS 链路，缓解通信盲区问题，被视为 6G 网络的关键技术之一。

近年来，学术界针对 RIS 辅助的无线通信系统开展了广泛的研究。然而，现有研究大多假设系统能获取理想 CSI，实际部署中这很难成立。因为 RIS 由无源器件构成，缺少独立射频链路，无法主动收发与处理导频信号；而且估计基站-RIS-用户级联信道的开销随RIS单元数量的增加而急剧增加，且通常需修改现有通信协议，阻碍了 RIS 的灵活部署。
为规避显式的 CSI 估计，一些研究提出了基于RSSI的盲波束赋形技术\cite{arun2020rfocus,ren2023configuring}。但面对数百乃至上千阵元的大规模 RIS，这些算法需要大量随机测量样本，寻优时间长，并且在全局随机配置探测期间用户难以正常传输数据。
另外，现有 RIS 波束赋形研究以数值仿真为主，缺乏针对复杂多径环境和商用现网环境的系统级现场试验。现网中宽波束与全向天线对 RIS 波束增益的稀释效应，也缺乏深入的实验评估。

针对上述问题，本章研究了支持在线传输的低复杂度盲波束赋形算法，并自主研制了大规模 RIS 原型系统，开展了多场景的实测验证。本章的主要研究内容如下：

\begin{enumerate}
	\item 提出了两种基于RSSI反馈的低复杂度盲波束赋形算法。首先，利用平面阵列的空间波束结构，设计了GFB算法以压缩搜索空间。为增强多径环境下的鲁棒性并充分利用多比特量化自由度，提出了ASG算法，通过动态调整扰动范围提升了算法跳出局部最优的能力，兼顾了收敛速度与寻优性能，且支持边传输边优化。
	
	\item 研制了两套工作于不同频段的大规模 RIS 原型系统。系统实现了从射频收发、功率测量到状态反馈与阵列控制的完整闭环，其控制策略无需修改现有通信协议。在实验室环境下验证了 RIS 信道的互易性，并在微波暗室中测试了其波束调控能力。
	
	\item 开展了多场景的实测验证与现网应用评估。在室内穿墙、 L 型走廊盲区和办公区场景下，验证了所提算法的收敛速度与覆盖增益；在实验室环境下，横向对比了各算法的实际增益，验证了所提 ASG 算法的性能优势。最后，在室外商用 5G 网络中进行覆盖增强实测，评估了 RIS 在全向天线终端与宽波束基站等真实组网条件下面临的增益损失，为规模化商用部署提供了实验参考。
\end{enumerate}

本章剩余部分组织如下：第~\ref{sec:ris_system_model}节介绍离散相移约束的 RIS 辅助下行无线通信系统模型，并阐述基于接收功率优化的盲波束赋形问题；第~\ref{sec:ris_algorithms}节介绍所提 GFB 算法与 ASG 算法的设计思想与流程；第~\ref{sec:ris_sim}节通过数值仿真验证所提算法的有效性及鲁棒性；第~\ref{sec:ris_prototype}节介绍所搭建的 RIS 原型通信系统的硬件架构设计及电磁特性验证结果；第~\ref{sec:ris_field_trials}节展示在室内盲区以及商用 5G 现网等场景下的实测结果与性能评估；第~\ref{sec:ris_conclusion}节对本章研究内容进行总结。


\section{系统模型}\label{sec:ris_system_model}
...
为克服上述困难，本章研究无需显式获取 CSI 的盲波束赋形方法，利用用户端易于获取的参考信号接收功率作为优化的依据。由于测量过程不可避免地包含噪声，用户端测得的 RSSI 的期望值可表示为信号功率与噪声功率之和：
\begin{equation}\label{eq:ris_rssi_definition}
	\mathbb{E}\left[\text{RSSI}(\bm{\phi})\right] = \mathbb{E}\left[|y|^2\right] = P_t \left| h_d + \bm{h}_r^\mathrm{T} \bm{\phi} \right|^2 + \sigma^2.
\end{equation}
对比发现，最大化 RSSI 在数学上与最大化 SNR 是完全等价的。于是，RIS 控制器可以周期性地调整其反射配置 $\bm{\phi}$，并根据用户反馈的 RSSI 来评估该配置的优劣，进而迭代地搜索最优解，实现在线优化。这一策略无需修改现有通信协议，具有良好的工程可实现性。下一节将针对这一问题模型，提出基于分组反馈的波束赋形算法，以在有限的反馈开销下高效地寻找最优的 RIS 配置。


\section{基于接收功率反馈的自适应波束赋形算法}\label{sec:ris_algorithms}


如上一节所述，将 RIS 的相移优化转化为最大化 RSSI 的盲优化问题，避开了较为困难的信道状态信息获取环节。然而，由式~\eqref{eq:ris_optimization_problem}可知，若直接在整个离散可行域内进行穷举搜索，其解空间大小高达 $(2^b)^N$。对于包含数百上千个反射单元的 RIS 面板而言，在实际系统较短的信道相干时间内无法穷举。因此，本节将提出两种启发式的自适应波束赋形算法。
...
快速贪婪波束赋形算法正是受此二维DFT码本空间量化特性的启发。为压缩搜索空间，GFB 放弃了对单个反射单元的独立盲搜，转而利用阵列的行列几何结构进行降维。为便于直观表述，将原一维反射系数向量 $\bm{\phi}$ 重新映射为反映二维物理位置的矩阵 $\bm{\Phi} \in \mathbb{C}^{N_z \times N_y}$。GFB 算法通过按行或按列对相移进行 $180^\circ$的整体翻转来执行局部搜索，整个算法分为水平搜索与垂直搜索两个连续阶段。算法首先以一个全部相同相位的初始矩阵启动，并记录初始的 RSSI 测量值。随后，逐列或逐行生成探测矩阵并下发至 RIS 面板。若探测状态下的 RSSI 较历史最佳值有所提升，则保留该翻转操作并更新最佳状态；反之，则拒绝翻转，恢复至上一状态。
...
\begin{algorithm}[!htbp]
	...
	\label{alg:ris_gfb}
	\begin{algorithmic}[1]
		\REQUIRE RIS 面板行数 $N_z$ 与列数 $N_y$，接收端实时反馈的RSSI测量值 $P(\cdot)$。
...
\section{数值仿真分析}\label{sec:ris_sim}
...
\end{equation}
其中，$d_\mathrm{BR}$ 和 $d_\mathrm{UR}$ 分别表示基站至RIS和RIS至用户的距离。基站到用户的背景信道的路径损耗则计算为：
\begin{equation}
...
\section{面向 6G 的 RIS 原型通信系统设计}\label{sec:ris_prototype}
...
这一结果证明了由无源超材料构成的RIS信道在宏观上服从电磁波的互易性定理。在时分双工模式下，上位机利用下行链路测得的RSSI迭代优化出的最优RIS配置向量$\bm{\phi}$，可直接应用于用户的上行信号传输。
...
\section{RIS原型系统实测与现网应用评估}\label{sec:ris_field_trials}
...
在不部署RIS的情况下，当用户设备在室内时，基站小区不在软件显示的小区列表中。部署RIS后，用户能够解码基站的物理小区标识，但此时的RSSI仍低于 $-90$ dBm。随后，系统运行GFB算法在线优化 RIS 反射波束。波束对准后，室内终端的 RSSI 比 RIS 关闭时提高了4至5 dB。
...
\section{本章小结}\label{sec:ris_conclusion}
...
本章围绕可重构智能表面在实际部署中面临的两大核心挑战——无法获取显式信道状态信息与缺乏系统级实测验证——展开研究，提出了低复杂度盲波束赋形算法，并自主研制了 RIS 原型通信系统，在多类真实场景中开展了实测验证。

在算法设计方面，本章提出了两种基于 RSSI 反馈的盲波束赋形算法，规避了代价高昂的级联信道估计流程。GFB 算法利用均匀平面阵列的二维 DFT 结构对搜索空间进行降维，将测量开销从 $\mathcal{O}(N_z \times N_y)$ 压缩至 $\mathcal{O}(N_z + N_y)$，并通过单调递增的贪婪保留准则保证了在波束对齐过程中不中断数据传输。ASG 算法放弃了对行列几何结构的依赖，引入余弦衰减的动态步长机制，在迭代初期以大步长实现广域探索，后期以小步长完成精细收敛，在多径散射与噪声干扰等复杂环境下表现出更强的鲁棒性与跳出局部最优的能力。数值仿真结果表明，在 256 单元、1 比特量化的系统配置下，ASG 算法能够在 1000 次测量内将可达信噪比提升至约 46 dB，逼近完美 CSI 上界；在量化比特数增加时性能稳步提升，在低莱斯因子的富散射信道下仍能维持良好的搜索效率，在大规模阵列下同样展现出较强的可扩展性。

在硬件实现方面，本章研制了工作于 5.8 GHz 频段的 1100 单元与工作于 2.6 GHz 频段的 256 单元两套 RIS 原型系统，均实现了从射频收发、功率测量到 RSSI 状态反馈与阵列控制的完整闭环，其控制策略无需对现有通信协议作任何修改。实测表明，包含 1100 个单元的面板总功耗仅为 0.93 W，具备低成本大规模部署的硬件基础。VNA 对 S 参数的实测结果证实了 RIS 信道在幅相两维度上均满足互易性，支持在 TDD 模式下复用下行优化结果用于上行传输。微波暗室的方向图测试表明，RIS 在 $30^\circ$ 至 $60^\circ$ 范围内的波束增益平坦度不足 1 dB，副瓣电平平均低于主瓣 9.8 dB，验证了原型系统的精确波束调控能力。

在实测验证方面，本章在多类典型场景下对原型系统与所提算法进行了全面评估。在室内穿墙非视距场景中，经 GFB 算法优化的 RIS 较同尺寸纯铜板提供了 26 dB 的接收功率增益，表明 RIS 在短距离 NLoS 场景下同样具有实用价值。在 L 型走廊盲区中，入射角为 $30^\circ$ 时可取得约 20 dB 的覆盖增益，且 GFB 算法通常在两次迭代内完成收敛，具备边传输边优化的能力。在室内办公区多点测试中，RIS 在三种部署位置下均可带来 15.5 dB 至 22 dB 的平均接收功率增益。在受控实验室环境的横向对比测试中，ASG 算法以平均 11.9 dB 的功率增益优于 DFT 算法（10.5 dB）和 GFB 算法（10.2 dB），与仿真结论一致。最后，在商用 5G 现网的覆盖增强实测中，RIS 可使室内终端 RSSI 提升 4 至 5 dB；进一步的对比实验揭示，基站宽波束与用户全向天线引入的增益稀释效应是现网条件下实测增益明显低于理想实验室条件的主要原因，这一发现为 RIS 走向规模商用的系统设计提供了重要的实验参考。

\noindent\rule{\linewidth}{2pt}

Gemini 1

本章面向可重构智能表面（RIS）在实际部署中面临的级联信道估计开销巨大以及离散量化约束等工程挑战，提出了基于接收功率反馈的盲波束赋形算法，并自主研制大规模原型系统完成了多场景实测评估。

在算法层面，利用平面阵列的波束响应几何特征，提出了快速贪婪波束赋形（GFB）算法，将搜索复杂度大幅降至 $\mathcal{O}(N_y+N_z)$；为克服局部最优陷阱并提升多径环境下的鲁棒性，进一步设计了自适应步长贪婪（ASG）算法。仿真表明，ASG 算法在无显式 CSI 的条件下能获取逼近理论上界的波束增益，且在不同莱斯因子与低比特量化环境中，其收敛性与稳定性显著优于现有的条件样本均值盲优化算法。

在工程实现与实测层面，研制了工作于 5.8 GHz（1100 单元）和 2.6 GHz（256 单元）的两套 1-bit 软硬件协同 RIS 原型系统，其实际运行功耗仅为 0.93 W。微波暗室测试验证了 RIS 具备良好的信道互易性与精确的波束指向控制能力。在复杂的室内物理环境中，所提方案展现出极强的补盲效果：在穿墙非视距场景下较同尺寸铜板取得 26 dB 的额外增益；在走廊与办公区边缘盲区分别实现了高达 14~20 dB 与 15.5~22 dB 的功率提升。同时，算法实测对比确认了 ASG 算法（平均增益 11.9 dB）优于 GFB 算法（10.2 dB）。

最后，在商用 5G 现网盲区测试中，RIS 成功实现了 4~5 dB 的覆盖增强，并通过对比实验揭示了全向天线接收复杂环境多径导致 RIS 相对增益稀释的物理机制。本章的软硬件联合设计与现网实测数据，为 RIS 技术向 6G 商用网络的规模化部署提供了重要的实践基础与可行性验证。

降AI

本章针对 RIS 实际部署中级联信道估计开销大、离散量化受限等工程问题，研究基于接收功率反馈的盲波束赋形算法，并研制大规模原型系统开展实测。
算法上，利用平面阵列的波束响应特征，提出 GFB 算法，搜索复杂度降至 $\mathcal{O}(N_y+N_z)$。为克服局部最优、增强多径鲁棒性，进一步设计了 ASG 算法。仿真表明，无显式 CSI 时 ASG 可获得接近理论最优的波束增益；在不同莱斯因子与低比特量化环境下，其收敛性和稳定性均优于条件样本均值算法。
工程实现上，研制了 5.8 GHz（1100 单元）和 2.6 GHz（256 单元）两套 1-bit RIS 原型，实际功耗 0.93 W。微波暗室测试表明 RIS 信道满足互易性，波束指向控制精度符合预期。室内复杂环境中，所提方案补盲效果明显：穿墙 NLoS 场景较同尺寸铜板增益高 26 dB；走廊与办公区边缘盲区分别提升 14~20 dB 和 15.5~22 dB。算法对比实测中，ASG 平均增益 11.9 dB，优于 GFB 的 10.2 dB。
商用 5G 现网盲区测试中，RIS 覆盖增强 4~5 dB。对比实验表明，全向天线接收多径信号导致 RIS 相对增益被稀释。本章的软硬件设计与现网实测数据，可为 RIS 向 6G 商用网络部署提供参考。

\noindent\rule{\linewidth}{2pt}

Gemini 2

本章针对可重构智能表面（RIS）在实际部署中面临的信道状态信息（CSI）获取困难与巨大导频开销难题，提出并评估了无需显式CSI的自适应盲波束赋形算法，并自主研制了大规模RIS原型通信系统开展了多场景的物理实测验证。

首先，本章将 RIS 的波束赋形转化为基于参考信号接收功率（RSRP）反馈的盲优化问题。为了在有限反馈开销下实现快速寻优，提出了快速贪婪波束赋形（GFB）算法与自适应步长贪婪（ASG）算法。GFB 算法利用二维平面阵列的空间响应结构，通过按行或按列的整体翻转机制，将搜索开销从 $\mathcal{O}(N)$ 骤降至 $\mathcal{O}(N_y+N_z)$，显著缩短了波束对齐延迟；ASG 算法则引入余弦衰减的动态步长机制，在迭代初期进行大范围探索以跳出局部最优，后期转入精细局部搜索。数值仿真表明，相较于传统的CSM等统计算法，所提算法在收敛速度和可达信噪比上具备显著优势，且在寻优期间支持“边传边优”模式。特别是 ASG 算法，在强多径环境和不同量化精度下均展现出极强的鲁棒性，最终性能紧逼完美 CSI 理论上界。

进一步地，本章搭建了软硬件协同的 RIS 原型通信系统，自主研制了分别工作于 5.8 GHz（1100单元）和 2.6 GHz（256单元）的 1-bit 低功耗 RIS 面板，其中1100单元的大规模阵列整机功耗不足 1 W。在微波暗室中，严格验证了 RIS 反射信道的互易性与准确的空间波束指向能力。在此基础上，将原型系统部署于真实的物理传播环境中开展了丰富的测试：室内穿墙与走廊边缘等非视距盲区测试表明，运行自适应波束赋形算法的 RIS 能够主动重构虚拟视距链路，为接收端带来最高 26 dB 的功率增益，有效消除了信号盲区；多算法对比实测也与理论仿真吻合，印证了 ASG 算法在真实复杂环境下的最优性能。

最后，本章将 RIS 原型接入商用 5G 现网开展了覆盖增强评估，实测使得室内盲区用户获得了 4~5 dB 的信号提升，并深入剖析了现网中由于用户端采用全向天线及环境强多径共同导致的增益稀释效应。综上所述，本章工作从盲波束赋形算法设计、硬件原型研制到多场景外场实测形成了完整的闭环，不仅验证了所提算法的有效性与鲁棒性，也为 RIS 技术在未来 6G 网络中的低成本、规模化商用落地提供了坚实的理论指导与极具价值的工程实践参考。